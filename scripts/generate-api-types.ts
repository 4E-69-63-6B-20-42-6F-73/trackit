...
const androidPath = (path: string) =>
    path === '/api/devices/pair/request' || path.startsWith('/api/device/')

const androidIdentifier = (path: string, method: string) =>
    `${path
        .replace(/^\/api\//, '')
        .replace(/\{([^}]+)\}/g, 'by_$1')
        .replace(/[^A-Za-z0-9]+/g, '_')
        .replace(/^_+|_+$/g, '')
        .toUpperCase()}_${method.toUpperCase()}`

const renderAndroidEndpoints = (document: JsonObject): string => {
    const paths = asObject(document.paths)
    if (!paths) throw new Error('OpenAPI document has no paths')
    const entries = Object.entries(paths)
        .filter(([path]) => androidPath(path))
        .sort(([left], [right]) => left.localeCompare(right))
        .flatMap(([path, rawPathItem]) => {
            ...
            return Object.entries(pathItem)
                ...
                .map(
                    ([method]) =>
                        `    val ${androidIdentifier(path, method)} = OpenApiEndpoint("${method.toUpperCase()}", "${path}")`,
                )
        })
    if (!entries.length) throw new Error('OpenAPI document has no Android device endpoints')
    return `package net.trackit.companion\n\n// Generated by scripts/generate-api-types.ts from the TrackIt OpenAPI contract.\n// Do not edit manually.\ndata class OpenApiEndpoint(\n    val method: String,\n    val path: String,\n)\n\nobject OpenApiEndpoints {\n${entries.join('\n')}\n}\n`
}

const webTarget = new URL('../src/lib/api.generated.ts', import.meta.url)
const androidTarget = new URL(
    '../android/app/src/main/java/net/trackit/companion/OpenApiEndpoints.generated.kt',
    import.meta.url,
)
...
try {
    ...
    const webSource = await format(renderPaths(document), {
        parser: 'typescript',
        printWidth: 100,
        tabWidth: 4,
        useTabs: false,
    ...
    const androidSource = renderAndroidEndpoints(document)
    if (process.argv.includes('--check')) {
        const currentWeb = await readFile(webTarget, 'utf8').catch(() => '')
        const currentAndroid = await readFile(androidTarget, 'utf8').catch(() => '')
        if (currentWeb !== webSource || currentAndroid !== androidSource)
            throw new Error('Generated API clients are out of date. Run npm run api:types.')
    } else {
        await Promise.all([
            writeFile(webTarget, webSource),
            writeFile(androidTarget, androidSource),
        ])
    }
} finally {
    await app.close()

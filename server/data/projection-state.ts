...
export { markProjectionDatesDirty, markProjectionDirty } from './projection-queue.js'
...
export class ProjectionWorker {
    ...
    private async initialize() {
        try {
            await new DailyProjectionCoordinator(this.database).reconcile()
        } catch (error) {
            this.log.error({ error }, 'Projection reconciliation failed')
        }
        await this.runOnce()
    }
